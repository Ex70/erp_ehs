...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1><i class="fas fa-tag mr-2"></i>Marcas</h1>
...
@section('content')
    ...
    <div class="card">
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge badge-info">
                                    {{ $m->asignaciones()->count() }}
                                </span>
                            </td>
                            <td>
                                <button class="btn btn-warning btn-xs"
                                        onclick="abrirEditar({{ $m->id }}, '{{ addslashes($m->nombre) }}', {{ $m->activo ? 1 : 0 }})">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <form action="{{ route('sistemas.marcas.destroy', $m) }}"
                                      method="POST" class="d-inline"
                                      onsubmit="return confirm('¿Eliminar {{ addslashes($m->nombre) }}?')">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-danger btn-xs"
                                            {{ $m->asignaciones()->count() > 0 ? 'disabled title=Tiene asignaciones activas' : '' }}>
        ...
        <div class="card-footer">{{ $marcas->links() }}</div>
    </div>

    {{-- Modal Nuevo --}}
    <div class="modal fade" id="modal-nuevo" tabindex="-1">
        <div class="modal-dialog">
            <form action="{{ route('sistemas.marcas.store') }}" method="POST">
                @csrf
